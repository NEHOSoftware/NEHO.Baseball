namespace NEHO.Baseball.Constants
{
    public class BaseballConstants
    {
        //public const string NEHOBaseballAPI = "http://nehobaseballapi.azurewebsites.net/";
        public const string NEHOBaseballAPI = "http://localhost:37655/";
        public const string ExpenseTrackerClient = "http://localhost:27470/";
        public const string ExpenseTrackerMobile = "ms-app://s-1-15-2-467734538-4209884262-1311024127-1211083007-3894294004-443087774-3929518054/";
    }
}
